...
| 平台 | 是否有积分计划 | 拉新激励 | 备注 |
|------|--------------|---------|------|
| **predict.fun** | ✅ Predict Points（被称唯一在跑积分的预测市场） | 积分 + 邀请 10% + 赛事奖池 | 空投预期是核心拉新引擎 |
| Polymarket | 历史有季度/积分活动 | 邀请返佣（达 $10k 门槛后分 30% 手续费） | 体量大、变现路径多 |
| Kalshi | 无（合规交易所） | 现金 welcome 促销 | 走传统获客 |
| Probable（已并入） | 上线零费 + 积分 | 已转入 Predict Points | — |

> predict.fun「唯一在跑积分计划 + YZi/Binance 背书 + 触达 200M Binance 用户」三者叠加，使其成为预测市场赛道最清晰的「空投农场」标的，但也意味着增长高度绑定 TGE 兑现预期。
